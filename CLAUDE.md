...
- `src/app/(app)/` — authenticated pages (chat, timeline, insights, profile). Middleware redirects unauthenticated users to `/auth/login`.
...
2. **Admin client** (inline `createSupabaseClient(url, SERVICE_ROLE_KEY)`) — bypasses RLS. Used for all server-side writes (log entries, profile updates, conversation messages). Created per-request, not as a singleton.

### AI stack — critical version constraint

The app uses two separate Anthropic packages:

- **`@ai-sdk/anthropic@1.2.12` + `ai@4.3.19`** — Vercel AI SDK. Used in all three AI API routes for `streamText`, `generateObject`, `createDataStreamResponse`.
...
| Route | Model | Purpose |
|---|---|---|
| `/api/chat` | claude-sonnet-4-5 | Main logging chat. Streaming. Two modes: onboarding (one tool: `complete_onboarding`) and regular (two tools: `search_food`, `create_log_entry`). Max 4 tool steps per response. |
| `/api/insights-chat` | claude-sonnet-4-5 | Insights Q&A. Streaming. No tools — reads 7–90 days of log entries and streams analysis. |
| `/api/quick-trend` | claude-haiku-4-5 | Returns one JSON trend object (`{ found, trend, confidence }`). Non-streaming, `generateObject`. |

### System prompts

`src/lib/prompts/chat.ts` — `buildChatSystemPrompt()` is called on every request. It injects the user's profile, goals, and today's full log. The prompt switches between onboarding mode and regular mode based on `profile.onboarding_complete`.
...
`profiles.quick_log_buttons` is a JSONB array of `{ id, emoji, label, message, enabled }` objects. An empty array means "use app defaults" — never null.

All timestamps in the database are UTC (`timestamptz`). The user's timezone is stored in `profiles.timezone` and used throughout to format times for display and to compute local-day boundaries for queries.
